...
import { standupStateManager, StandupAnswers } from '@standup-scribe/core';
import { STEP_CONFIG, STANDUP_STEPS, StandupStep, QUICK_DATE_OPTIONS } from '@standup-scribe/core';
...
export async function handleStandupInteraction(
  ...
  const rosterMember = await prisma.rosterMember.findUnique({
    ...
    include: { excusals: true, workspace: true },
  });
...
function renderStep(
  ...
  if (config.isDate) {
    // Quick date buttons
    const dateButtons = QUICK_DATE_OPTIONS.map((date: string) =>
      new ButtonBuilder()
  ...
  if (config.isSelect && config.options) {
    const selectButtons = config.options.map((opt: string) =>
      new ButtonBuilder()
